...
export class OngoingOrdersComponent implements OnInit {
  ...
  private fetchFromApi(isRefresh = false): void {
    ...
    this.authservice.checkAuth().pipe(
      catchError(err => { this.logger.error('[Ongoing] checkAuth:', err); return of(null); })
    ).subscribe(response => {
      ...
      this.detailservice.getorderdata(response.user.ID).pipe(
        catchError(err => { this.logger.error('[Ongoing] getorderdata:', err); return of(null); }),
 ...
 private applyFilter(): void {
  this.orderlist.set(
    this.cachedAllOrders.filter(o => {
      const status = o.Status?.toLowerCase();
      return status === 'active' || status === 'ongoing' || status === 'in progress' || status === 'order  ongoing';
    })
  );
}
  ...
  private done(isRefresh: boolean): void {
    if (isRefresh) {
      ...
      // ✅ Skeleton shows for minimum 2 seconds for a polished feel
