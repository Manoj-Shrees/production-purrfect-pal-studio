...
onImageLoad() {
  ...
  private moveAction(e: PointerEvent) {
    ...
    if (this.dragging) {
      this.targetLeft = Math.min(Math.max(minX, this.currentLeft + dx), maxX - this.currentWidth);
      this.targetTop = Math.min(Math.max(minY, this.currentTop + dy), maxY - this.currentHeight);
    }

    if (this.resizing && this.currentHandle) {
      if (this.currentHandle.includes('r')) this.targetWidth += dx;
      if (this.currentHandle.includes('l')) {
        this.targetLeft += dx;
        this.targetWidth -= dx;
      }
      if (this.currentHandle.includes('b')) this.targetHeight += dy;
      if (this.currentHandle.includes('t')) {
        this.targetTop += dy;
        this.targetHeight -= dy;
      }
    }

    this.targetWidth = Math.max(50, this.targetWidth);
    this.targetHeight = Math.max(50, this.targetHeight);
