...
export class StripeService {
  ...
  public static readonly SUPPORTED_CURRENCIES: Record<string, { code: string; symbol: string; isZeroDecimal?: boolean }> = {
    // North America
    usd: { code: 'usd', symbol: '$' },
   ...
   */
  static async handleWebhookEvent(rawBody: Buffer, signatureHeader: string): Promise<{ received: boolean; eventType: string }> {
  ...
  private static async handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
    ...
    const planTier = (session.metadata?.planTier as any) || 'monthly';
    ...
    await dbPool.execute(
      `INSERT INTO customers (id, email, name, stripe_customer_id)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE stripe_customer_id = VALUES(stripe_customer_id), name = VALUES(name)`,
      [`cust_${session.id.substring(8, 24)}`, email.toLowerCase().trim(), name, stripeCustomerId]
    );
  ...
  private static async handleSubscriptionUpdated(subscription: Stripe.Subscription) {
    ...
    if (periodEnd) {
      await dbPool.execute(
        `UPDATE licenses SET expires_at = ?, status = ? WHERE subscription_id = ?`,
        [periodEnd, status, stripeSubId]
      );
    }
  }
  ...
  private static async handleSubscriptionDeleted(subscription: Stripe.Subscription) {
    ...
    await dbPool.execute(
      `UPDATE licenses SET status = 'expired', revocation_reason = 'Stripe subscription canceled' WHERE subscription_id = ?`,
      [stripeSubId]
    );
  }
  ...
  private static async handleInvoicePaymentSucceeded(invoice: Stripe.Invoice) {
    ...
    if (subId) {
      ...
      if (periodEnd) {
        await dbPool.execute(
          `UPDATE licenses SET expires_at = ?, status = 'active' WHERE subscription_id = ?`,
          [periodEnd, subId]
        );
      }
    }
  ...
  private static async handleInvoicePaymentFailed(invoice: Stripe.Invoice) {
    ...
    if (subId) {
      await dbPool.execute(
        `UPDATE licenses SET status = 'suspended', revocation_reason = 'Payment renewal failed' WHERE subscription_id = ?`,
        [subId]
      );
    }
  }
   ...
   */
  static async createCustomerPortalSession(customerId: string, returnUrl?: string): Promise<{ portalUrl: string }> {
  ...
  static async getCheckoutSessionStatus(sessionId: string): Promise<{
    ...
    if (session.payment_status === 'paid' || session.status === 'complete') {
      let [rows] = await dbPool.execute<RowDataPacket[]>(
        `SELECT license_key, plan_type, status FROM licenses WHERE email = ? ORDER BY created_at DESC LIMIT 1`,
